...
<template>
  <main class="min-h-screen p-12">
    <h2 class="text-4xl font-bold text-center mb-12 m-auto">
      Programma Selectie - SV de Rijp
    </h2>
    ...
    <table class="m-auto">
      ...
      <tbody>
        ...
        >
          ...
          >
            <NuxtImg
              :src="`https://logoapi.voetbal.nl/logo.php?clubcode=${item.opponentClubCode}`"
              :alt="`Clublogo van ${item.opponent}`"
              class="inline-block max-w-6 max-h-6 mr-2"
            />
            {{ item.opponent }}
          </td>
          <td>{{ item.time }}</td>
