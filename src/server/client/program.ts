...
import type { Match, EventType, Duration, MatchStatus } from '~/types/match'
...
const getDuration = (teamNameWithClubName: string): Duration => {
  const teamName = teamNameWithClubName.replace(`${clubName} `, '')
  const category = teamName.split('-')[0] || teamName

  const duration = ageCategoryMatchDurations.find(
...
export const getProgram = async ({
  ...
  const matches = result
    .filter(item => new Date(item.wedstrijddatum).getTime() > now)
    .map(item => ({
...
const fixTime = (date: Date) => {
  ...
  const dutchTime = new TZDate(date, 'Europe/Amsterdam')
...
export const getPrograms = async ({
  ...
  const programs = await Promise.all(
    teamCodes.map(teamCode => getProgram({ days, teamCode, home, away })),
  )
