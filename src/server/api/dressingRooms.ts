...
import type { DressingRoom } from '~/server/client/dressingRooms'
import { getDressingRooms } from '~/server/client/dressingRooms'
...
const assignRooms = (matches: Match[], dressingRooms: DressingRoom[]) => {
  ...
  const assignments = []
  for (const [roomName, teams] of roomAssignments) {
    assignments.push({
      ...teams[0],
      room: roomName,
    })
